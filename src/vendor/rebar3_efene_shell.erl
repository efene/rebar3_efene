...
init(State) ->
    State1 = rebar_state:add_provider(
            ...
            providers:create([
                ...
                {bare, false},
                {deps, ?DEPS},
                {example, "rebar3 efene shell"},
                {short_desc, "Run efene shell with project apps and deps in path."},
                {namespace, efene},
                {desc, info()},
...
setup_new_shell(ShellArgs) ->
    ...
    case ShellArgs of
        undefined ->
            _ = user_drv:start(["tty_sl -c -e", {fn_repl, start, []}]);
        _ ->
...
rewrite_leaders(OldUser, NewUser) ->
    ...
    %% liveness check.
    _ = [try erlang:group_leader(NewUser, Pid) catch _:_ -> error end
    ...
    try
        ...
        E:R:S -> % may fail with custom loggers
